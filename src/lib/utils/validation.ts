...
export const validationMessages = {
    ...
    password: 'Пароль должен содержать минимум 6 символов, включая заглавные и строчные буквы, а также цифры',
    ...
    fileTooLarge: (maxSize: number) => `Размер файла не должен превышать ${maxSize} МБ`,
};
...
export const cosmetologistRegisterSchema = registerSchema.extend({
    ...
    graduationYear: z
        ...
        .max(new Date().getFullYear(), `Год не может быть больше ${new Date().getFullYear()}`),
    specialization: z
        .string()
        .max(255, validationMessages.maxLength(255))
        .optional(),
...
export const productSchema = z.object({
    ...
    cosmetologistPrice: z.number().min(0, validationMessages.minValue(0)).optional(),
    ...
    ingredients: z.string().max(5000, validationMessages.maxLength(5000)).optional(),
    usageInstructions: z.string().max(5000, validationMessages.maxLength(5000)).optional(),
...
export type CosmetologistRegisterInput = z.infer<typeof cosmetologistRegisterSchema>;
